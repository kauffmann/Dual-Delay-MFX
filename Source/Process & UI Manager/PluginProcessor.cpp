...
PluginProcessor::PluginProcessor()
    ...
    parameters(*this, nullptr, juce::Identifier("MFX"), createParameterLayout())  // *this allow APVTS to add parameters to audioProcessor, that how getParameters() works.
...
//----------------------------------------------------------------------------------------------------------------------------------------
    ...
    // This protect plugin from audio boost in DAW if something goes wrong. 
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
...
void PluginProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    //Serializes parameters, and any other potential data into an XML. This only goes into saved xml in DAW.
...
AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout()
{
    ...
    /*You don't get a compile error when returning a std::vector<std::unique_ptr<juce::RangedAudioParameter>> because AudioProcessorValueTreeState::ParameterLayout can be constructed from a range of iterators. When you return a vector, the compiler can implicitly convert it to a ParameterLayout using its range constructor.

Here's the key part:

cpp

// This works because ParameterLayout has a constructor that takes a pair of iterators
return { params.begin(), params.end() };

This range constructor allows you to pass iterators from a vector, making the conversion seamless and avoiding compilation errors.  But it is more readable to do
juce::AudioProcessorValueTreeState::ParameterLayout layout;

        // Define parameters and add them to the layout
        layout.add(std::make_unique<juce::AudioParameterFloat>("gain", "Gain", 0.0f, 1.0f, 0.5f));
        layout.add(std::make_unique<juce::AudioParameterFloat>("cutoff", "Cutoff", 20.0f, 20000.0f, 1000.0f));
        // Add more parameters as needed

        // Return the ParameterLayout
        return layout;*/
