    ...
    rasterComponent(p), mProcessor(p)
{
    ...
    // constrainer is used to control a components resizable options/constraints. When user mouse drag and resize window
    if (auto* constrainer = getConstrainer())
    {
        constrainer->setFixedAspectRatio(static_cast<double> (originalWidth) / static_cast<double> (originalHeight)); //important, is used when resized. w ad h must always fit this ratio
...
void WrappedRasterAudioProcessorEditor::resized()
{
    ...
    rasterComponent.setTransform(AffineTransform::scale(scaleFactor)); // this is the actual scale transforming
    //rasterComponent.setBounds(0, 0, originalWidth, originalHeight); // unintuitive, but need this as transform use original bounds to transform from.
