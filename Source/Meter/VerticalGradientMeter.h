...
#include <JuceHeader.h>
	...
	public:
		...
		void paint(Graphics& g) override
		{
			...
			g.fillRect(bounds);
			DBG("valueHeight" << static_cast<float>(getHeight()));
			g.setGradientFill(gradient);
			const auto scaledY = jmap(valueSupplier(), -100.0f, 6.0f, 0.0f, static_cast<float>(getHeight())); // was -60, but min in-valueSupplier is -100 
			DBG("value" << scaledY);
		...
		void resized() override
		{
            ...
            gradient.addColour(0.8, Colours::white);  // was 0.7
